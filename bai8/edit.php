...
<?php
        if(isset($_POST['btn-edit'])) {
            // Lấy thông tin từ form users
            $name = $_POST['name'];
            $email = $_POST['email'];
            $phone = $_POST['phone'];
            $addrees = $_POST['addrees'];

            if ($name != "" && $email != "" && $phone != "" && $addrees != "") {
                $sql = "UPDATE users SET name = :name, email = :email, phone = :phone, addrees = :addrees WHERE id = :id";
                $query = $pdo->prepare($sql);
                $query->execute(['name' => $name, 'email' => $email, 'phone' => $phone, 'addrees' => $addrees, 'id' => $id]);
...
<div class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-9">
                <input type="text" class="form-control" name="addrees"value="<?php echo $row['addrees'];?>"/>
            </div>
        </div>
